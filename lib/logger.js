...
function requestLogger (appName) {
  return function (req, res, next) {
    ...
    res.end = function (chunk, encoding) {
      ...
      var client = '???';
      if (req.user && req.user.name) client = req.user.name;
      else if (req.authInfo && req.authInfo.client && req.authInfo.client.name) client = req.authInfo.client.name;
      ...
      status + ' ' + req.method + ' ' + req.url + ' From ' + client + ' Auth: ' + req.get('Authorization') + ' in ' + elapsedTime + 'ms');
    };
    next();
  };
}
